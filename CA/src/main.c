...
/*  Set the CA folder connected to a port to allow it to act as a server that is "always running" simulating a network with device, CA, and powerHypervisor
        Then this can run separately and always be running if wanted

    

    Step 4: If valid step 3 then read CSR's CN (Common Name) SN (serial number) ensuring they are present, it matches with listed possibilities, and that it isn't already deployed or signed.
        could look to add key strength Ex. if key bit < 2048 reject "Not secure enough key"

    Step 5: If all steps are valid sign csr with private key and send new cert back to device. Then store signed cert and SN and CN name which it can use later to see if duplicate csr's are sent

    Step 6: If at any point something isn't valid reject it print a statement of the device name and why it's rejected, adn store it to certs/rejected which can be helpful to analyze what device were trying to connect
...
// Step 1: Create public key, private key, and cert for CA only one time
...
// Step 2: On powering up connect to a port
    // if user doesn't provide 2 arguments
    if (argc < 2) {
...
// Step 3: Loop a listen function to listen for csr request to the CA "port"
    while(keep_running) {
        ...
        if (newsockfd < 0) {
            ...
            if (errno == EINTR) continue; // Checks if error num from signal is interruption 
        ...
        if (val <= 0) {
            printf("Failed to receive size of expected data for the CA: DISCONNECTING\n\n");
            close(newsockfd);
            continue;
        }
        ...
        while (total < expect_file_size) {
            ...
            if (n <= 0) {
                printf("receive failed for the CA\n");
                free(buffer);
                close(newsockfd);
                close(sockfd);
                return ERROR;
    ...
    /* Step 4: Once received a CSR reads and Validate CSR was signed by private key of matching public key (integrity)
        Things needed to be checked by CA before the csr can get signed
            ...
            * Check serial number that it's in the database and unused
            * Device name that it's in the database and unused
    */

        // Writing csr to file to then convert file to openSSL req struct to perform functions on
    ...
    // Checking CSR fields compared to database
        ...
        while(fgets(db_buffer, sizeof(db_buffer), db_fp) != NULL) {
            ...
            // Technically not needed because of how strings were initalized but good practice
            org[index_org] = '\0';
            com_n[index_com_n] = '\0';
            sn[index_sn] = '\0';
    ...
    // Replacing old database with updated database
        ...
        if (is_device_approved) {
            ...
            printf("Sending Certificate back: \n\n");
