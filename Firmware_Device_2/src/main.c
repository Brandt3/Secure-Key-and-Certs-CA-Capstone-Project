...
// Possible later note look into making it so only device_2 can open the keys and certs file within 
...
// 1. Upon "Power On" Check if there are keys created if not Create Private and Public key (RSA or EC)
    ...
    const char key_name_priv[] = "Firmware_Device_2/keys/firmware2_priv_rsa_key.pem";
    const char key_name_pub[] = "Firmware_Device_2/keys/firmware2_pub_rsa_key.pem";
...
//  Check if public key fp exist / is created
        ...
        FILE *priv_key_fp = fopen("Firmware_Device_2/keys/firmware2_priv_rsa_key.pem", "r");
        if (!priv_key_fp) {
            fprintf(stderr, "Error opening private key file for device2 for public key creation\n");
        }
...
// 2. Create CSR and request user for CSR info plus exstention
    ...
    const char csr_fp[] = "Firmware_Device_2/certs/device2.csr";

    // If csr file doesn't exist create it
    if (!is_fp_Exist(csr_fp)) {
        FILE *priv_key_fp = fopen("Firmware_Device_2/keys/firmware2_priv_rsa_key.pem", "r");
        if (!priv_key_fp) {
            fprintf(stderr, "Error opening private key file for device2\n");
        }

        EVP_PKEY *pkey = PEM_read_PrivateKey(priv_key_fp, NULL, NULL, NULL);
        if (!pkey) {
            fprintf(stderr, "Creating pkey based of private key file reading failed for device 2\n");
        }
    ...
    // STEP 1: Load OpenSSL config from file
        ...
        const char config_path[] = "Firmware_Device_2/config/device2_csr.conf";
    ...
    // STEP 4: Get the subject name section from config
        ...
        if(!name) {
            fprintf(stderr, "Error loading config file subject name for firmware device2\n");
            return ERROR;
        }
    ...
    // STEP 8: Write CSR to file
        FILE *csr_fp = fopen("Firmware_Device_2/certs/device2.csr", "wb");
        if (!csr_fp) {
...
// All this is duplicate code that can be put into a function 
        ...
        EVP_PKEY *pkey = PEM_read_PrivateKey(priv_key_fp, NULL, NULL, NULL);
        if (!pkey) {
            fprintf(stderr, "Error creating pkey based of private key file reading failed for device 2\n");
        }
    ...
    // STEP 1: Load OpenSSL config from file
        // creates a OpenSSL config structure 
        CONF *conf = NCONF_new(NULL); 
        char config_path[] = "Firmware_Device_2/config/device2_csr.conf";
        if (NCONF_load(conf, config_path, NULL) <= 0) {
            ERR_print_errors_fp(stderr);
            return ERROR;
        }
...
// 3. Send CSR to CA to see if it will get signed
    ...
    if (connect(sockfd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) {
        printf("ERROR connecting to server on Device 2\n");    
        close(sockfd);
        return ERROR;
    }
    ...
    if (!fp) {
        printf("Error opening csr file on device 2\n");
        close(sockfd);
        return ERROR;
    }
    ...
    if (fseek(fp, 0, SEEK_END) == 0) {
        ...
        perror("Error accessing the end of file in device2\n");
        close(sockfd);
        return ERROR;
    }
    ...
    while (total_sent < bytes_read) {
        ...
        if (n <= 0) {
            printf("Error sending csr to device 2 over the socket\n");
            close(sockfd);
            return ERROR;
        }
        total_sent += n;
...
// === Receiving Data from CA ===
    ...
    const char *expected_cert_fp = "Firmware_Device_2/certs/device2_cert.crt";
...
// FIXME this code has not been tested yet
    ...
    if (is_cert_signed) {
        ...
        FILE *cert_fp = fopen("Firmware_Device_2/certs/device2_cert.crt", "wb");
        if (!cert_fp) {
    ...
    // FIXME This is a function that needs to be updated
        ...
        FILE *csr_fp = fopen("Firmware_Device_2/certs/device2.csr", "r");
        if (!csr_fp) {
        ...
        FILE *out = fopen("Firmware_Device_2/certs/device2_cert.crt", "w");
...
// 4. If it has a signed cert send it to the Power Hypervisor to try and connect
    ...
    FILE *cert_fp = fopen("Firmware_Device_2/certs/device2_cert.crt", "r");
    if (!cert_fp) {
